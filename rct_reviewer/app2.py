...
def create_bias_highlighted_pdf(pdf_bytes, annotations):
    ...
    for page_num, page in enumerate(doc):
        ...
        for ann in bias_annotations:
            ...
            if not text or len(text) < 10 or not bias_domain: continue
            ...
            try:
                ...
                for area in areas:
                    if area.y0 < header_height: continue
...
def export_to_csv(results):
    ...
    for r in results:
        ...
        for p in r.get("pico", []): row[f"pico_{p['domain'].lower()}"] = " | ".join(p.get("text", []))
        for b in r.get("bias", []): row[f"bias_{b['domain'].lower().replace(' ', '_')}"] = b.get("judgement", "N/A")
        rows.append(row)
...
def main():
    ...
    st.markdown(f"""
    **Running in Cloud Mode: Models are loaded from Hugging Face Hub.**
    ...
    with st.sidebar.expander("📋 How to Cite"):
        st.markdown("""**Marshall IJ, Kuiper J, & Wallace BC.** RobotReviewer...""")
    ...
    if st.button("🚀 Analyze Documents", type="primary"):
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            try:
                ...
                result = {
                    "filename": file.name, "pdf_bytes": pdf_bytes,
    ...
    if 'results' in st.session_state and st.session_state['results']:
        ...
        with col1: st.metric("Total Documents", len(results))
        with col2: st.metric("Identified RCTs", sum(1 for r in results if r['rct']['is_rct']))
        with col3: st.metric("Low Risk Assessments", sum(1 for r in results if any(b['judgement'] == 'low' for b in r['bias'])))
        with col4: st.metric("Documents with Issues", len(results) - sum(1 for r in results if r['rct']['is_rct']))
        ...
        for result in results:
            with st.expander(f"📄 {result['filename']}", expanded=True):
                ...
                with rct_col1: st.metric("Is RCT?", "✅ Yes" if rct['is_rct'] else "❌ No", delta=f"Score: {rct['score']:.3f}")
                with rct_col2: st.metric("Probability", f"{rct['probability']:.1%}")
                with rct_col3: st.metric("Model", rct.get('model', 'SVM'))
                ...
                for i, pico_domain in enumerate(["Population", "Intervention", "Outcomes"]):
                    with pico_cols[i]:
                        ...
                        if domain_data and domain_data.get('text'):
                            for sent in domain_data['text'][:top_k_sentences]: st.info(f"• {sent}")
                        else: st.caption("_No elements extracted_")
                ...
                if bias:
                    ...
                    for b in bias:
                        ...
                        bias_data.append({
                            "Domain": b['domain'], "Color": hex_color, "Judgement": judgement,
